...
u32 listing_registry_keys(u32 arg_count, u8 *command_line[])
{
	...
	keyIndex = options_tchar(arg_count, command_line, _T("Rl"), &flags[0], &flags[1], NULL);
	...
	for (	i = 0;
         ...
         i++) ;
	if (PreDefKeyNames[i] == NULL) report_error_tchar(_T("Use a Predefined Key"), 1, FALSE);
	hKey = PreDefKeys[i];
    ...
    {
		report_error_tchar(_T("Cannot open subkey properly"), 2, TRUE);
    }
	hKey = hNextKey;
    
	ok = traverse_registry(hKey, command_line[keyIndex], NULL, flags);
	RegCloseKey(hKey);
