...
u32 key_compare (LPCTSTR, LPCTSTR);
...
u32 memory_mapped_file_sorting (u32 arg_count, u8 * command_line[])
{
	...
	iFirstFile = Options (arg_count, command_line, _T ("n"), &NoPru32, NULL);
    
	if (arg_count <= iFirstFile)
		report_error (_T ("Usage: sortFL [options] files"), 1, FALSE);
    
	__try {	/* try-except */
		/* Copy the input file to a temp output file that will be sorted.
		Do not alter the input file. */
		_stpru32f_s (tempFile, MAX_PATH, _T ("%s.tmp"), command_line[iFirstFile]);
		CopyFile (command_line[iFirstFile], tempFile, TRUE);
		...
		__try {
			...
			/* Pru32 out the entire sorted file. Treat it as one single string. */
			pTFile = (u8 *) pFile;
			pTFile[fileSize.QuadPart/TSIZE] = _T('\0');
			if (!NoPru32)
				Pru32Msg (hStdOut, pFile);
		}
...
u32 key_compare (LPCTSTR pKey1, LPCTSTR pKey2)
