...
#define DLL_PUSHBACK(first, last, node) (((first)==0?\
(first)=(last)=(node):\
((last)->next=(node),(last)=(node))),\
(node)->next=(node)->prev=0)

#define DLL_PUSHFRONT(first, last, node) ...

#define DLL_REMOVE(first, last, node) (((first)==(node)?\
...
internal I1f32 I2f32_axis      (I2f32 r, Axis axis);
